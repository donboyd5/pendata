...
gdmatches <- left_join(gmatches, dmatches, by = join_by(name)) # 89 items

tofix <- gdmatches |>
  filter(don != gang) # 23 items out of 89 where my value does not match gang's

constants_vec <- unlist(frs$constants_assumptions)
# djb as of 12/12/2025 I actually do have ben_payment ratio so the next line may not be necessary
...
fglob$model_return_ <- 0.067
fglob$return_2023_ <- 0.067
...
# Load frs_data_env to extract needed objects
...
# Verify all needed objects exist in frs_data_env
missing_objects <- setdiff(needed, ns(frs_data_env))
if (length(missing_objects) > 0) {
  stop(
    "Missing objects in frs_data_env: ",
    paste(missing_objects, collapse = ", ")
  )
}

# Extract the needed objects from frs_data_env
needed_objects <- mget(needed, envir = frs_data_env, inherits = FALSE)

# Verify we got all objects
cat(
  "Successfully extracted",
  length(needed_objects),
  "objects from frs_data_env:\n"
)
print(names(needed_objects))
...
## Move top-level frs objects (except plan_shortname) into params_env and
## add Gang "needed" objects into params_env as individual entries

# Move every top-level element of `frs` (except `plan_shortname`) into params_env
top_names <- setdiff(names(frs), "plan_shortname")
for (n in top_names) {
  params_env[[n]] <- frs[[n]]
}

# Add the needed objects from frs_data_env into params_env as individual named elements
for (nm in names(needed_objects)) {
  params_env[[nm]] <- needed_objects[[nm]]
}

# Ensure no duplication of plan_shortname
if ("plan_shortname" %in% ls(params_env)) {
  rm(list = "plan_shortname", envir = params_env)
}

# Convert params_env to a list for reliable saving in package data
if (is.environment(params_env)) {
  params_env_list <- as.list(params_env, all.names = TRUE)
} else {
  params_env_list <- params_env
}

# Rebuild frs to only contain plan_shortname and params_env (as a list)
frs <- list(
  plan_shortname = frs$plan_shortname,
  params_env = params_env_list
)

# Save the updated frs object
fpath <- fs::path(GROOT, "data", "frs.rda")
save(list = "frs", file = fpath, compress = "xz")
cat("\nSaved frs.rda to:", fpath, "\n")
