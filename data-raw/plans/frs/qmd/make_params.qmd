...
# Add the needed objects from frs_data_env as individual named elements
frs$salary_headcount_table <- needed_objects$salary_headcount_table
frs$mort_table <- needed_objects$mort_table
frs$mort_retire_table <- needed_objects$mort_retire_table
frs$separation_rate_table <- needed_objects$separation_rate_table
frs$entrant_profile_table <- needed_objects$entrant_profile_table
frs$dr_lookup <- needed_objects$dr_lookup
frs$cola_lookup <- needed_objects$cola_lookup
frs$ben_mult_lookup <- needed_objects$ben_mult_lookup
frs$reduce_factor_lookup <- needed_objects$reduce_factor_lookup
frs$tier_table <- needed_objects$tier_table
frs$fas_period_lookup <- needed_objects$fas_period_lookup

# Verify all needed objects are now in frs
cat("\nVerifying all needed objects are in frs:\n")
for (obj_name in needed) {
  if (obj_name %in% names(frs)) {
    cat("  ✓", obj_name, "\n")
  } else {
    stop("ERROR: Missing object in frs: ", obj_name)
  }
}
cat("\nAll", length(needed), "needed objects successfully added to frs!\n")
