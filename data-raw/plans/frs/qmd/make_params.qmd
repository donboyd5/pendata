...
## Load and categorize the "target" params data from Gang
```{r}
...
### Work on the stacked tables

```{r}
#| label: headcount_table_
...
tables <- list()
tables$salary_table_ <- prep |> 
  select(age, class, yos_lb, salary)
```
  
  
  ## Construct params from parts
  
  ```{r}

  fixed_matches # matching globals, fixed
fglob # missing globals
# to come: # missing tables
# to come: # other

# params that SHOULD match gang's params -- paramsg
# note that I am NOT forcing types to match
params <- c(fixed_matches, fglob)
...
## Load and categorize the "target" frs_data_env_bf_cal data from Gang
```{r}
#| label: get-frs-gang
#| eval: false
