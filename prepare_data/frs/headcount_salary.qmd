...
test_that("calculated salaries equal reported salaries by age group", {
  # TO COME
  # NOTE: This will be important if we construct additional details for age groups or yos groups
  salaries <- hcs_tbl |>
    # filter(class == "judges", yos_label == "Under 5") |>
    pivot_wider(names_from = variable) |>
    mutate(payroll = headcount * salary) |>
    summarise(
      headcount = sum(headcount),
      payroll = sum(payroll),
      salary_reported = first(salary),
      .by = c(class, age_label, rectype)
    ) |>
    mutate(
      salary_computed = replace_na(payroll / headcount, 0),
      diff = salary_computed - salary_reported
    ) |>
    arrange(class, age_label, rectype) # |>
  # select(class, age_label, rectype, salary) |>
  # pivot_wider(names_from = rectype, values_from = salary, values_fill = 0)

  expect_equal(salaries$detail, salaries$total, tolerance = 1e-8)
  salaries |>
    mutate(diff = detail - total) |>
    arrange(desc(abs(diff)))
})

tmp <- hcs_tbl |> filter(class == "admin", variable == "salary")

# what about when salary is average salary for the table?
